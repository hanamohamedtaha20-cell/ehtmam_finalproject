const String baseUrl = 'https://graduation-production-b4c1.up.railway.app';

const String signupEndpoint            = '/userlog/signup';
const String loginEndpoint             = '/userlog/login';
const String logoutEndpoint            = '/userlog/logout';
const String forgotPasswordEndpoint    = '/userlog/forgotpassword';
const String updatePasswordEndpoint    = '/userlog/updatepassword';
const String caregiverSignupEndpoint   = '/caregiver/signup';
const String caregiverEndpoint         = '/caregiver';
const String servicesEndpoint          = '/services';
const String requestEndpoint           = '/request';
const String availableRequestsEndpoint = '/request/available';
const String offerEndpoint             = '/offer';
const String bookingFromOfferEndpoint  = '/booking/bookingfromoffer';
const String bookingEndpoint           = '/booking';
const String bookingCheckinEndpoint    = '/booking/checkin';
const String createPaymentEndpoint     = '/payment/create';
const String paymentCallbackEndpoint   = '/payment/callback';
const String paymentRedirectEndpoint   = '/payment/redirect';
const String payBookingWalletEndpoint  = '/payment/pay-booking-wallet';
const String processPaymentEndpoint = '/booking/process-payment';
const String walletEndpoint            = '/wallet';
const String transactionEndpoint       = '/transaction';
const String tasksEndpoint             = '/tasks';
const String reviewEndpoint            = '/review';
const String bundleEndpoint            = '/bundle/get-all-bundle';
const String clientBundleEndpoint      = '/clientbundle';
const String chatMessageEndpoint       = '/chat/message';
const String chatHistoryEndpoint       = '/chat/history';
const String chatSessionsEndpoint      = '/chat/sessions';
const String newChatSessionEndpoint    = '/chat/sessions/new';
const String adminPendingCaregiversEndpoint = '/admin/caregivers/pending';
const String adminCaregiversEndpoint = '/admin/caregivers';
const String adminBlockEndpoint = '/admin/block';
const String createBundleEndpoint = '/bundle/create_bundle';
const String updateBundleEndpoint = '/bundle/update_bundle';
const String deleteBundleEndpoint = '/bundle/delete_bundle';
const String allTransactionsEndpoint = '/transaction/all_transactions';
const String allUsersEndpoint = '/userlog/allusers';
const String myWalletEndpoint = '/wallet/my-wallet';
const String clientBundlePayEndpoint    = '/clientbundel/payBundle';
const String myActiveBundleEndpoint     = '/clientbundle/my-active-bundle';
const String acceptWithBundleEndpoint   = '/clientbundle/use-for-booking';
const String notificationsEndpoint = '/notifications';
const String complaintsEndpoint      = '/complaints';
const String adminComplaintsEndpoint = '/admin/complaints';
const String myReviewsEndpoint       = '/review/my-reviews';